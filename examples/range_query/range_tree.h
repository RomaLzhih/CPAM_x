...
timer init_tm, build_tm, total_tm, aug_tm, sort_tm, reserve_tm, outmost_tm,
    globle_tm, g_tm;
double early;
...
struct RangeQuery
{
   ...
   struct inner_map_t
   {
      ...
      using val_t = w_type;
      static bool
      comp( key_t a, key_t b )
      {
         return a < b;
      }
      using aug_t = w_type;
      inline static aug_t
      from_entry( key_t k, val_t v )
      {
         return v;
      }
      inline static aug_t
      combine( aug_t a, aug_t b )
      {
         return a + b;
      }
      static aug_t
   ...
   struct outer_map_t
   {
      ...
      {
         auto ret =
             aug_t::map_union( std::move( a ), std::move( b ),
                               []( w_type x, w_type y ) { return x + y; } );
         return ret;
      }
      static aug_t
      get_empty()
   ...
   struct count_t
   {
      ...
      void
      add_entry( ENTRY_TYPE<point_x, w_type> e )
      {
         if( std::get<0>( e ).second >= y1.first &&
             std::get<0>( e ).second <= y2.first )
            r += 1;
      }
      void
      add_aug_val( inner_map a )
      {
   ...
   w_type
   query_count( x_type x1, y_type y1, x_type x2, y_type y2 )
   {
   ...
   w_type
   query_sum( x_type x1, y_type y1, x_type x2, y_type y2 )
   {
   ...
   struct range_t
   {
      point_y y1, y2;
      point_y* out;
      range_t( point_y y1, point_y y2, point_y* out )
          : y1( y1 ), y2( y2 ), out( out )
      {
      }
      void
      add_entry( ENTRY_TYPE<point_x, w_type> e )
      {
         ...
         {
            *out++ =
                point_y( std::get<0>( e ).second, std::get<0>( e ).second );
         }
      }
      void
      add_aug_val( inner_map a )
   ...
   sequence<point_y>
   query_range( x_type x1, y_type y1, x_type x2, y_type y2 )
   {
      // std::cout << "Calling query count on " << x1 << " " << y1 << " " << x2
      // << " " << y2 << std::endl;
   ...
   void
   insert_point_lazy( point_type p )
   {
      range_tree = outer_map::insert_lazy(
          std::move( range_tree ), make_pair( make_pair( p.x, p.y ), p.w ) );
   }
   ...
   {
      cout << "Outer Map: ";
      outer_map::GC::print_stats();
      cout << "Inner Map: ";
      inner_map::GC::print_stats();
   ...
   size_in_bytes()
   {
      ...
      size_t internal_nodes_space =
          ( sizeof( typename outer_map::node ) * outer_used ) +
          ( sizeof( typename inner_map::node ) * inner_used );
#else
      size_t internal_nodes_space =
          ( sizeof( typename outer_map::GC::regular_node ) * outer_used ) +
          ( sizeof( typename inner_map::GC::regular_node ) * inner_used );
#endif
