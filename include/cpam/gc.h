...
struct gc
{
   ...
   static void
   reserve( size_t n, bool randomize = false )
   {
      alloc::reserve( n / B );
      // complex_alloc::reserve(n/B);
      //  std::cout << "Reserving.... " << (n/B) << " nodes with size = " <<
      //  sizeof(regular_node) << std::endl;
      // std::cout << "Reserving.... " << (n/B) << " complex nodes with size = "
      // << sizeof(typename Node::complex_bytes) << std::endl;
   }
   static void
   finish()
   {
   ...
   // copy node if reference count is > 1, incrementing the children's ref
   // counts then decrements the copied node's count does not update
   static inline node*
   copy_if_needed( node* t )
   {
   ...
   static inline void
   dec_if( node* t, bool copy, bool extra_ptr )
   {
      if( !extra_ptr )
      {
         if( copy )
   ...
   struct ptr
   {
      ...
      // extra_ptr = true if there is an implicit extra pointer to this node,
      // but we don't want to bump the ref-cnt to encode this explicitly.
      // Instead, we store a bit to remember that there's an extra pointer.
      ...
      static node*
      set_flag( node* x, bool f )
      {
         return (node*)( ( (size_t)x ) + f );
      }
      static bool
      has_flag( node* x )
      {
      ...
      is_compressed()
      {
         return Node::is_compressed( strip_flag( p ) );
      }
      bool
      is_regular()
      {
         return Node::is_regular( strip_flag( p ) );
      }
   };
};
